module.exports.config = {
  ...
  module.exports.run = async function ({ event, api , args, Users}){
    ...
    var msg = `[卍] RANDOM IMAGE [卍]\n\n`;
    var x = 1;
    for (var i = limit * (page - 1); i < limit * (page - 1) + limit; i++) {
        if (i >= b) break;
        msg += `[卍] ${i+1} [卍] → ${name[i]}\n\n`;
    }
    msg += `[卍] Trang (${page}/${numPage})\n[卍] Dùng ${global.config.PREFIX}${this.config.name} <số trang>\n\n[卍] Xin mời ${name1} reply tin nhắn theo số thứ tự để chọn ảnh!!!`;
